...
local function fake_peripherals(methods)
  return {
    wrap = function() return methods end,
    methods = function()
      local out = {}
...
return {
  test_build_schedule_from_stops_uses_create_format = function()
    local schedule = create_train_schedule.build_schedule({ { from = "A", to = "B", route_id = "R1", dwell_seconds = 10 } })
  ...
  test_build_schedule_prefers_explicit_create_destination = function()
    local schedule = create_train_schedule.build_schedule({ { to = "ST-B", create_destination = "Hauptbahnhof B" } })
    assert(schedule.entries[1].instruction.data.text == "Hauptbahnhof B")
  end,

  test_build_schedule_resolves_station_registry_destination = function()
    local registry = { get = function(id) if id == "ST-B" then return { create_station_name = "Hauptbahnhof B" } end end }
    local schedule = create_train_schedule.build_schedule({ { to = "ST-B" } }, { station_registry = registry })
    assert(schedule.entries[1].instruction.data.text == "Hauptbahnhof B")
  end,
  ...
  test_build_schedule_fails_without_destination = function()
    ...
    assert(schedule == nil)
    assert(err == "missing Create destination/station name")
  end,

  test_apply_uses_create_station_set_schedule = function()
    local applied = nil
    local adapter = create_train_schedule.new(fake_peripherals({ isTrainPresent = function() return true end, setSchedule = function(schedule) applied = schedule end }), { station = "Create_Station_0" })
  ...
  test_apply_stops_builds_and_applies_schedule = function()
    local applied = nil
    local adapter = create_train_schedule.new(fake_peripherals({ isTrainPresent = function() return true end, setSchedule = function(schedule) applied = schedule end }))
  ...
  test_apply_fails_when_no_train_present = function()
    local adapter = create_train_schedule.new(fake_peripherals({ isTrainPresent = function() return false end, setSchedule = function() error("should not be called") end }))
