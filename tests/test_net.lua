...
return {
 ...
 test_retry_after_time=function() local c=clock(); local m=mock_modem.new(); local a=net.new(m,1,"A",log.new("INFO",10),{now_ms=function() return c.now end}); local msg=a.send_reliable("cmd","B",{}); assert(a.pending[msg.id].retry==0); a.tick(); assert(a.pending[msg.id].retry==0); c.now=300; a.tick(); assert(a.pending[msg.id].retry==1) end,
