...
function trains.new(config)
  ...
  function self.assign_route(train_id, route_id, destination)
  ...
  function self.mark_offline(train_id)
    if by_id[train_id] then
      by_id[train_id].state = STATES.OFFLINE
    end
  end

  function self.get(train_id)
    return by_id[train_id]
  end

  function self.list()
    local out = {}
    for id, train in pairs(by_id) do
      out[id] = copy(train)
    end
    return out
  end
