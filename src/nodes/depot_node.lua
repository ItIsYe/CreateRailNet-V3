  ...
  for _, track in ipairs(depot_config.tracks or depot_config.slots or {}) do
    ...
    if id then
      tracks[id] = {
        ...
        kind = track.kind or track.type or depot_config.depot_type or "mixed",
        ...
        ready_after_seconds = track.ready_after_seconds or depot_config.ready_after_seconds or 5,
        occupied_since = nil,
        last_occupied = false
      }
    end
...
function depot_node.render_status(monitor, state)
  ...
  monitor.write("CreateRailNet Depot")
  monitor.setCursorPos(1, 3)
  monitor.write("Depot: " .. tostring(state.depot_id or state.node_id or "-"))
  monitor.setCursorPos(1, 4)
  monitor.write("Type: " .. tostring(state.depot_type or "mixed"))
  monitor.setCursorPos(1, 5)
  monitor.write("State: " .. tostring(state.state or "ONLINE"))
  local row = 7
  ...
  monitor.write("Queue: " .. tostring(#(state.queue or {})))
end

local function track_payload(state, track)
  return {
    type = "depot_track_status",
...
function depot_node.new_runtime(args_or_context)
  ...
  local state = {
    ...
    state = "ONLINE",
    queue = {},
    tracks = depot_node.build_track_map(depot_cfg)
  }
  ...
  local node = common_node.new({
    ...
    handlers = {
      on_cmd = function(payload)
        ...
        if cmd == "reserve_track" and track then
          track.state = "RESERVED"
          track.train_id = payload.train_id
          track.route_id = payload.route_id
          track.destination = payload.destination
        elseif cmd == "clear_track" and track then
          track.state = TRACK_EMPTY
          track.train_id = nil
          track.route_id = nil
          track.destination = nil
          track.occupied_since = nil
        elseif cmd == "stage_train" and track then
          track.state = TRACK_STAGING
          track.train_id = payload.train_id or track.train_id
          track.route_id = payload.route_id or track.route_id
          track.destination = payload.destination or track.destination
        ...
        elseif cmd == "dispatch_train" and track then
          ...
          node.net.send("event", context.config.master_id, {
            type = "depot_request_dispatch",
            depot_id = depot_id,
            track_id = track.id,
            train_id = track.train_id,
            route_id = payload.route_id or track.route_id,
            destination = payload.destination or track.destination
          })
        elseif cmd == "set_depot_message" then
          state.message = payload.message
        else
          return false, "unknown depot cmd: " .. tostring(cmd)
        end
  ...
  local function send_track(track)
    node.net.send("event", context.config.master_id, track_payload(state, track))
  end
  ...
  node.start = function()
    old_start()
    depot_node.render_status(monitor, state)
    node.depot_timer = os.startTimer(0.5)
    node.net.send("event", context.config.master_id, {
      type = "depot_status",
      depot_id = depot_id,
      depot_type = depot_type,
  ...
  node.handlers.on_event = function(event)
    if event[1] == "timer" and event[2] == node.depot_timer then
      check_tracks()
      depot_node.render_status(monitor, state)
      node.depot_timer = os.startTimer(0.5)
    end
...
if args.id then
  depot_node.new_runtime(bootstrap.create_context({...}, "depot")).run()
end
