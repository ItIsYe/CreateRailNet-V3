  ...
  for _, platform in ipairs(station_config.platforms or station_config.tracks or {}) do
    ...
    if id then
      platforms[id] = {
        ...
        kind = platform.kind or platform.type or station_config.station_type or "mixed",
        sensor_id = platform.sensor_id,
        block_id = platform.block_id,
        dwell_seconds = platform.dwell_seconds or station_config.dwell_seconds or 15,
        state = PLATFORM_EMPTY,
        train_id = nil,
        occupied_since = nil,
        last_occupied = false
      }
...
function station_node.render_status(monitor, state)
  ...
  monitor.write("CreateRailNet Station")
  monitor.setCursorPos(1, 3)
  monitor.write("Station: " .. tostring(state.station_id or state.node_id or "-"))
  monitor.setCursorPos(1, 4)
  monitor.write("Type: " .. tostring(state.station_type or "mixed"))
...
function station_node.new_runtime(args_or_context)
  ...
  local state = {
    ...
    state = "ONLINE",
    platforms = station_node.build_platform_map(station_cfg)
  }
  ...
  local node = common_node.new({
    ...
    handlers = {
      on_cmd = function(payload)
        ...
        if cmd == "reserve_platform" and platform then
          platform.state = "RESERVED"
          platform.train_id = payload.train_id
        elseif cmd == "clear_platform" and platform then
          platform.state = PLATFORM_EMPTY
          platform.train_id = nil
          platform.occupied_since = nil
        elseif cmd == "mark_ready_departure" and platform then
          platform.state = PLATFORM_READY
          platform.train_id = payload.train_id or platform.train_id
          node.net.send("event", context.config.master_id, {
            type = "station_ready_departure",
            station_id = station_id,
            platform_id = platform.id,
            train_id = platform.train_id,
        ...
        elseif cmd == "set_station_message" then
          state.message = payload.message
        else
          return false, "unknown station cmd: " .. tostring(cmd)
        end
  ...
  local function send_platform(platform)
    node.net.send("event", context.config.master_id, platform_payload(state, platform))
  end
  ...
  node.start = function()
    old_start()
    station_node.render_status(monitor, state)
    node.station_timer = os.startTimer(0.5)
    node.net.send("event", context.config.master_id, {
      type = "station_status",
      station_id = station_id,
      station_type = station_type,
  ...
  node.handlers.on_event = function(event)
    if event[1] == "timer" and event[2] == node.station_timer then
      check_platforms()
      station_node.render_status(monitor, state)
      node.station_timer = os.startTimer(0.5)
    end
...
if args.id then
  station_node.new_runtime(bootstrap.create_context({...}, "station")).run()
end
