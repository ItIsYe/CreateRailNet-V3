...
local function present_names()
  ...
  if peripheral and peripheral.getNames then
    ...
    for side in pairs(SIDES) do if peripheral and peripheral.isPresent and peripheral.isPresent(side) then out[side] = true end end
  end
  return out
end
...
function report.build(config_path)
  ...
  for _, node in ipairs(cfg.nodes or {}) do
    ...
    table.insert(rows, { id = node.id, role = node.role, binding = binding, exists = exists == true, type = info and info.type or nil, method_count = info and info.method_count or nil, warning = binding and nil or "no binding field" })
  end
