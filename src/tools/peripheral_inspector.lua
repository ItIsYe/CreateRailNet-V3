...
local function print_line(text) print(tostring(text)) end
local function sort_list(list) table.sort(list, function(a, b) return tostring(a) < tostring(b) end); return list end

local function list_names()
  if peripheral and peripheral.getNames then return sort_list(peripheral.getNames()) end
  local names = {}
  for _, side in ipairs(DEFAULT_SIDES) do if peripheral and peripheral.isPresent and peripheral.isPresent(side) then table.insert(names, side) end end
  return names
end
...
local function contains_any(text, patterns)
  ...
  for _, pattern in ipairs(patterns or {}) do if string.find(lower, string.lower(pattern), 1, true) then return true end end
  return false
end
...
function inspector.inspect(name)
  ...
  local hints = {}
  for _, method in ipairs(methods) do if contains_any(method, CREATE_HINTS) then table.insert(hints, method) end end
  return { name = name, type = safe_type(name), present = wrapped ~= nil, method_count = #methods, methods = methods, create_hints = hints }
end

function inspector.scan()
  local report = { generated_at = time.now_s(), peripherals = {} }
...
function inspector.find_methods(patterns)
  ...
  for _, info in ipairs(inspector.scan().peripherals) do
    for _, method in ipairs(info.methods or {}) do
      if contains_any(method, patterns or CREATE_HINTS) then table.insert(matches, { name = info.name, type = info.type, method = method }) end
    end
  end
  return matches
end
...
function inspector.write_report(path, report)
  ...
  if fs and fs.open then
    local fh = fs.open(target, "w"); fh.write(body); fh.close(); return true, target
  end
  if io and io.open then
    local fh = io.open(target, "w")
    if not fh then return false, "cannot open " .. tostring(target) end
    fh:write(body); fh:close(); return true, target
  end
...
function inspector.run(args)
  ...
  local report = options.side and { generated_at = time.now_s(), peripherals = { inspector.inspect(options.side) } } or inspector.scan()
  ...
  if options.write then local ok, result = inspector.write_report(options.write, report); print_line((ok and "written: " or "write failed: ") .. tostring(result)) end
  return report
end
