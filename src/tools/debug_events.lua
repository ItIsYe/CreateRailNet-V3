...
local function build_payload(args)
  if args.kind == "sensor" then return { type = "sensor", sensor_id = args.sensor_id, action = args.action or "enter" }
  elseif args.kind == "train_depart" then return { type = "request_departure", train_id = args.train_id, from = args.from, to = args.to, route_id = args.route_id }
  elseif args.kind == "train_arrived" then return { type = "arrived", train_id = args.train_id, station = args.station, route_id = args.route_id }
  elseif args.kind == "manual" then return { type = "manual_control", action = args.action, train_id = args.train_id, route_id = args.route_id, signal_id = args.signal_id, switch_id = args.switch_id, aspect = args.aspect, position = args.position, reason = args.reason } end
  return nil, "unknown debug kind: " .. tostring(args.kind)
end

debug_events.build_payload = build_payload
