...
function setup_wizard.build_config(report, opts)
  ...
  add_node(cfg.nodes, { id = options.from_station or "ST-A", role = "station", station_type = options.kind or "passenger", display_name = options.from_station or "ST-A", monitor = monitor and monitor.info.name or "monitor", platforms = { { id = "P1", kind = options.kind or "passenger", sensor_id = "OBS-GEN-1", block_id = "B-GEN-1", dwell_seconds = options.dwell_seconds or 5 } } })
  add_node(cfg.nodes, { id = options.to_station or "ST-B", role = "station", station_type = options.kind or "passenger", display_name = options.to_station or "ST-B", monitor = monitor and monitor.info.name or "monitor", platforms = { { id = "P1", kind = options.kind or "passenger", sensor_id = "OBS-GEN-2", dwell_seconds = options.dwell_seconds or 5 } } })
